class VideoPlayer {
  constructor(selector, options) {
    ...
    this.video = document.getElementById(this.selector);
    this.parent = null;
    this.elements = {};
  ...
  check() {
    if (!this.video) {
      console.error('There is no element with this selector');
      return;
    }

    if (!this.video.getAttribute('src')) {
      console.error("The src attribute isn't filled in");
      return;
    }
  }
      ...
      `
        <div class="video-player__volume-indicator hidden">0%</div>
          <div class="video-player__controls">
            <div class="video-player__timeline timeline" tabindex="0" role="slider" aria-label="Video slider"
              aria-valuemin="0" aria-valuemax="" aria-valuenow="0" aria-valuetext="">
              <div class="timeline__progress"></div>
              <div class="timeline__handle"></div>
            </div>
            <div class="video-player__bottom">
              <button class="video-player-btn video-player__play" type="button" aria-keyshortcuts="k"
                aria-label="To activate the Watch button, press k">
              ...
              <div class="video-player__volumes">
                <button class="video-player-btn video-player__volume" type="button" aria-keyshortcuts="m"
                  aria-label="To activate the Mute button, press m">
                  <span class="video-player-btn__volume-icon video-player-btn__icon video-player-btn__icon--unmuted"></span>
                </button>
                <div class="video-player-volume-panel" role="slider" tabindex="0" aria-valuemin="0" aria-valuemax="100"
                  aria-valuenow="" aria-valuetext="" aria-label="Громкость">
              ...
              <button class="video-player-btn video-player__fullscreen" type="button" aria-keyshortcuts="f"
                aria-label="To activate the Full Screen button, press f">
  ...
  findElements() {
    ...
    this.elements = {
      controls: parent.querySelector('.video-player__controls'),
      ...
      function () {
        ...
        this.setAria(
          ...
          `${this.formatTime(
            currentTime,
            'dot',
          )} (total duration ${this.formatTime(duration, 'dot')})`,
        );
  ...
  updateTimelineAria() {
    ...
    this.setAria(
      this.elements.timeline,
      'aria-valuetext',
      `${this.formatTime(currentTime, 'dot')} (total duration ${this.formatTime(
        duration,
        'dot',
      )})`,
    );
  ...
  initVolume() {
    const newHandleDistance =
      this.options.initialVolume * this.maxHandleDistance;
  ...
  updateVolume(event, changeVolume) {
    ...
    if (
      ...
      let offsetX =
        event.clientX - this.elements.volumePanel.getBoundingClientRect().left;

      newHandleDistance = Math.min(
        this.maxHandleDistance,
        Math.max(0, offsetX - this.volumeHandleWidth / 2),
      );
    ...
    if (event.type === 'keydown') {
      if (changeVolume === 'inc') {
        volume = Math.min(1, volume + this.options.updateVolume);
      } else if (changeVolume === 'dec') {
        volume = Math.max(0, volume - this.options.updateVolume);
      }
    ...
    this.setAria(
      this.elements.volumePanel,
      'aria-valuetext',
      `${(volume * 100).toFixed(0)}% volume${mutedText}`,
    );
  }
  ...
  play() {
    ...
    this.elements.playButton.setAttribute(
      'aria-label',
      'To activate the Pause button, press k',
    );
    this.elements.playButtonIcon.classList.replace(
      'video-player-btn__icon--play',
      'video-player-btn__icon--pause',
    );
  }
  ...
  pause() {
    ...
    this.elements.playButton.setAttribute(
      'aria-label',
      'To activate the Watch button, press k',
    );
    this.elements.playButtonIcon.classList.replace(
      'video-player-btn__icon--pause',
      'video-player-btn__icon--play',
  ...
  mute() {
    ...
    this.elements.volumeBtn.setAttribute(
      'aria-label',
      'To activate the Unmute button, press m',
    );
    this.elements.volumeButtonIcon.classList.replace(
      'video-player-btn__icon--unmuted',
      'video-player-btn__icon--muted',
    );
  }
  ...
  unmute() {
    ...
    this.elements.volumeBtn.setAttribute(
      'aria-label',
      'To activate the Mute button, press m',
    );
    this.elements.volumeButtonIcon.classList.replace(
      'video-player-btn__icon--muted',
      'video-player-btn__icon--unmuted',
  ...
  events() {
    ...
    this.elements.volumeBtn.addEventListener(
      'click',
      function () {
        if (!this.muted) {
          this.mute();
    ...
    this.video.addEventListener(
      ...
      function () {
        ...
        this.elements.timelineHandle.style.transform =
    ...
    this.elements.volumePanel.addEventListener(
      'mousedown',
      function (event) {
